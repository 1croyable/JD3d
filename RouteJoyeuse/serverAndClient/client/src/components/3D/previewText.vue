...
function adjustCameraAndControlPosition(camera, controls) {
    // 计算包围盒
    const bbox = new THREE.Box3().setFromObject(toRaw(props.previewTextModel.object3D));
...
onMounted(() => {
    ...
    // 添加文本模型到场景中
    if (props.previewTextModel) {
        const textModel = toRaw(props.previewTextModel.object3D);
        scene.add(textModel);
    ...
    // 监听props.updateAnimate变化
    watch(() => props.updateAnimate, () => {
        renderScene();
    });

    watch(() => props.previewTextModel, () => {
        if (props.previewTextModel) {
            adjustCameraAndControlPosition(camera, controls);
        }
    }, { deep: true })
